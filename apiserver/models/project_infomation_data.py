...
class ProjectInfomationModel(db.Model, CRUD):
    ...
    pro_name          = db.Column(db.String(128), comment = u'楼盘名称')
    pro_address       = db.Column(db.String(128), comment = u'楼盘地址')
    pro_company       = db.Column(db.String(128), comment = u'楼盘开发商')
    pro_ave_price     = db.Column(db.Float,       comment = u'楼盘平均价格')
    pro_sale_card     = db.Column(db.String(64),  comment = u'楼盘预售证')
    ...
    pro_lng           = db.Column(db.Float,       comment = u'楼盘经度')
    pro_lat           = db.Column(db.Float,       comment = u'楼盘纬度')
    pro_sale_date     = db.Column(db.DateTime,    comment = u'楼盘开始售卖日期')

    pro_province_uuid = db.Column(db.String(32), db.ForeignKey('t_province_dict.province_uuid'),     comment = u'外键，关联楼盘所属省份')
    pro_province      = db.relationship('ProvinceModel', backref = db.backref('project'),    lazy = 'dynamic')
    pro_city_uuid     = db.Column(db.String(32), db.ForeignKey('t_city_dict.city_uuid'),         comment = u'外键，关联楼盘所属城市')
    pro_city          = db.relationship('CityModel', backref = db.backref('project'),        lazy = 'dynamic')
    pro_district_uuid = db.Column(db.String(32), db.ForeignKey('t_district_dict.district_uuid'),     comment = u'外键，关联楼盘所属区域')
