...
class ProjectInfomationModel(db.Model, CRUD):
    ...
    pro_lng           = db.Column(db.Double, comment = '楼盘所在经度')
    pro_lat           = db.Column(db.Double, comment = '楼盘所在纬度')
    pro_price         = db.Column(db.Double, comment = '楼盘均价')
    pro_total_area    = db.Column(db.Double, comment = '楼盘上市总面积')
    pro_area_sold     = db.column(db.Double, comment = '楼盘已售面积')
    pro_area_selling  = db.column(db.Double, comment = '楼盘在售面积')
    ...
    province          = db.relationship('ProvinceModel', backref = db.backref('houses'), laze = 'dynamic')
    city_uuid         = db.Column(db.String(32), db.ForeignKey('t_city_dict.uuid'), comment = '外键, 关联城市')
    city              = db.relationship('CityModel', backref = db.backref('houses'), laze = 'dynamic')
    district_uuid     = db.Column(db.String(32), db.ForeignKey('t_district_dict.uuid'), comment = '外键, 关联城市行政区')
    district          = db.relationship('DistrictModel', backref = db.backref('houses'), laze = 'dynamic')
